...
export const fetchAllOrders = async (pageNumber = 1, searchQuery = '') => {
    const token = await AsyncStorage.getItem(TOKEN_KEY); // Retrieve the token
    ...
    const url = `${BASE_URL}/api/orders/?${params.toString()}`;
  
    // console.log("Fetching orders for page:", pageNumber, "URL:", url);
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            return response.data.data; // Return the fetched data directly if the request is successful
        } else {
...
export const deleteOrders = async (id) => {
    ...
    console.log(`Deleting order with ID: ${id} at URL: ${url}`);
    ...
    try {
        ...
        if (response.status === 200) { // 204 No Content indicates successful deletion
            console.log("Delete response:", response);
            return response; // Adjust this according to your actual API response structure
        } else {
            throw new Error(`Failed to delete orders: Status code ${response.status}`);
        }
    } catch (error) {
        console.error("Error deleting orders:", error);
        throw new Error("Failed to delete order due to network or server error.");
    }
};
...
export const saveCustomer = async (name, phone, secondary_phone, town, region) => {
    const token = await AsyncStorage.getItem(TOKEN_KEY); 
...
export const fetchAllCustomer = async (pageNumber = 1, searchQuery = '') => {
    const token = await AsyncStorage.getItem(TOKEN_KEY);
    ...
    const url = `${BASE_URL}/api/customer/?${params.toString()}`;
    ...
    //   console.log('fetchAllCustomer response:', response.data);
...
export const editCustomer = async (name, phone, secondary_phone, town, region, id) => {
    const token = await AsyncStorage.getItem(TOKEN_KEY); 
